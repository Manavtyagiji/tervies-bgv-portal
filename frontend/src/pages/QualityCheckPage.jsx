...
import { generateBGVReportAsBase64 } from "../utils/generateReport";
...
`;
// QualityCheckPage — 3-COLUMN LAYOUT
...
// It is only for on-screen preview in Quality Check. Download can still use PDF.
// ─────────────────────────────────────────────────────────────────────────────
...
function buildQualityCheckHtmlPreview(data = {}, previewImages = []) {
...
<style>
  * { box-sizing: border-box; }
  ...
  .brand-logo { width: 58px; height:58px; border-radius: 14px; background:#1d4ed8; color:#fff; display:flex; align-items:center; justify-content:center; font-size:24px; font-weight:800; }
...
async function createHtmlPreviewBlobUrl(reportData) {
  ...
  const html = buildQualityCheckHtmlPreview(reportData || {}, previewImages);
...
// IndexedDB so local preview works without filling localStorage. Quality Check
...
async function getQCManualCasesFromDB() {
  try {
    ...
    console.warn("QualityCheck IndexedDB read skipped:", e);
    return [];
  }
}
...
// ─────────────────────────────────────────────────────────────────────────────
// HELPER — sendReportToCRM (ORIGINAL — unchanged)
// ─────────────────────────────────────────────────────────────────────────────
async function sendReportToCRM(report) {
  try {
    ...
    const res = await fetch(`${API_BASE}/admin/send-to-crm`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
    ...
    if (!res.ok || !data.success) {
      throw new Error(data.message || `CRM API failed with status ${res.status}`);
    }
        ...
        <div style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: "column" }}>
          ...
          {selected && (
            <div style={{ padding: "6px 12px", borderBottom: "1px solid #e2e8f0", background: "#f8fafc", display: "flex", alignItems: "center", gap: 6, flexShrink: 0, flexWrap: "wrap" }}>
              ...
              <button onClick={() => setZoom(z => Math.min(2, +(z + 0.25).toFixed(2)))} style={btnStyle}>＋</button>
              ...
              <span style={{ fontSize: 10, color: "#94a3b8", minWidth: 34, textAlign: "right" }}>{Math.round(zoom * 100)}%</span>
            </div>
          )}
          {/* Preview content */}
...
function DocPreview({ file, zoom }) {
  ...
  // Case document preview works like report preview:
  // 100% shows the complete image inside the panel using object-fit: contain.
  // Zoom out makes the full image smaller. Zoom in allows scroll, without cropping the image element.
  const safeZoom = Math.max(0.25, Math.min(Number(zoom) || 1, 2));
  const isZoomedIn = safeZoom > 1;
  ...
  return (
    ...
    >
      {isImage ? (
        <div
          style={{
            width: isZoomedIn ? `${safeZoom * 100}%` : "100%",
            height: isZoomedIn ? `${safeZoom * 100}%` : "100%",
            minWidth: isZoomedIn ? `${safeZoom * 100}%` : 0,
            minHeight: isZoomedIn ? `${safeZoom * 100}%` : 0,
            display: "flex",
            alignItems: "center",
        ...
        >
          <img
            ...
            style={{
              width: "auto",
              height: "auto",
              maxWidth: isZoomedIn ? "none" : `${Math.round(Math.min(safeZoom, 0.95) * 100)}%`,
              maxHeight: isZoomedIn ? "none" : `${Math.round(Math.min(safeZoom, 0.95) * 100)}%`,
              objectFit: "contain",
        ...
        <iframe
          ...
          title={name}
          style={{
            width: isZoomedIn ? `${safeZoom * 100}%` : `${Math.round(safeZoom * 100)}%`,
            height: isZoomedIn ? `${safeZoom * 100}%` : `${Math.round(safeZoom * 100)}%`,
            minWidth: isZoomedIn ? 500 : 320,
            minHeight: isZoomedIn ? 700 : 520,
            maxWidth: isZoomedIn ? "none" : "100%",
            maxHeight: isZoomedIn ? "none" : "100%",
            border: "none",
            borderRadius: 10,
            boxShadow: "0 8px 24px rgba(15,23,42,0.18)",
            background: "#fff",
...
// ─────────────────────────────────────────────────────────────────────────────
// MAIN COMPONENT — QualityCheckPage
// ─────────────────────────────────────────────────────────────────────────────
export default function QualityCheckPage() {
  ...
  const loadCases = async () => {
    try {
      const token = localStorage.getItem("token") || localStorage.getItem("adminToken");
      const [localManualCases, serverResult] = await Promise.all([
        getQCManualCasesFromDB(),
        fetch("https://tervies.info/api/admin/cases", {
      ...
      const map = new Map();
      serverCases.forEach(c => map.set(String(c.caseId), c));
      localManualCases.forEach(localCase => {
        const key = String(localCase.caseId || "");
        if (!key) return;
        const serverCase = map.get(key) || {};
        map.set(key, mergeManualDocsIntoReport(serverCase, localCase));
      });
      setAllReports(Array.from(map.values()));
    } catch (err) {
      console.error("Error loading cases:", err);
      const localManualCases = await getQCManualCasesFromDB();
      if (localManualCases.length) setAllReports(localManualCases);
    }
  };
  ...
  const handleSearch = () => {
    ...
    if (matched) {
      ...
      navigate(`/quality-check/${encodeURIComponent(matched.caseId)}`, { replace: true, state: { reportData: matched } });
    } else {
  ...
  const alreadySent   = sentCases[currentCaseId] || report?.sentToCRM;
  ...
  const createPdfBlobFromReport = async () => {
    ...
    // so Quality Check preview/download must call generateBGVReportAsBase64().
    ...
    try {
      ...
      if (!savedReport && !report?.savedAt) {
        try {
          ...
          console.warn("Auto-save before CRM send failed:", saveErr);
          // Continue anyway — we can still send to CRM with current report data
        }
      }

      const success = await sendReportToCRM(report);
      if (success) {
        setSentCases((prev) => ({ ...prev, [report.caseId]: true }));
        setSendSuccess(true);
      } else {
        alert("Failed to send report to CRM. Please try again.");
      }
    } catch (error) {
      console.error("Send to CRM error:", error);
      ...
      <div className="qc-wrap">
          ...
          <div className="qc-actions">
            {report && (
              <>
                ...
                {showSendButton && (
                  alreadySent ? (
                    <div className="qc-btn-sent">✓ Sent to CRM</div>
                  ) : (
                    <button className="qc-btn-send" onClick={handleSendToCRM} disabled={isSending}>
                      {isSending ? (
                        <><svg style={{ width: 16, height: 16, animation: "qcSpin 0.8s linear infinite" }} viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="white" strokeWidth="4" opacity="0.3" /><path fill="white" d="M4 12a8 8 0 018-8v8z" /></svg> Sending...</>
                      ) : <>🚀 Send to CRM</>}
                    </button>
                  )
                )}
        ...
        {sendSuccess && (
          <div className="qc-send-banner">
            ...
            <div>
              <div className="qc-send-banner-title">Report Sent to CRM!</div>
              <div className="qc-send-banner-desc">
                The background verification report for <strong>{report?.name}</strong> (Case ID: <strong style={{ fontFamily: "monospace" }}>{report?.caseId}</strong>) has been approved and sent to CRM.
              </div>
        ...
        {alreadySent && report && !sendSuccess && (
          <div className="qc-already-sent-banner">
            ...
            <span>
              This report has already been sent to CRM.
              This case is now available in CRM Check.
            </span>
          </div>
        )}
 
        {/* ── Adjustable panel width controls ─────────────────────────────── */}
        <div className="qc-card" style={{ padding: "6px 14px", marginBottom: 8, borderRadius: 14 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, marginBottom: 3, flexWrap: "wrap" }}>
            <div>
              <div style={{ fontWeight: 800, color: "#334155", fontSize: 12, letterSpacing: "0.04em", textTransform: "uppercase", lineHeight: 1.1 }}>Adjust Panel Width</div>
              <div style={{ color: "#94a3b8", fontSize: 10, marginTop: 1, lineHeight: 1.15 }}>Drag sliders to adjust Case Numbers, Report Preview, and Case Documents.</div>
            </div>
            <button onClick={resetPanelLayout} style={{ background: "#fff", color: "#475569", border: "1px solid #cbd5e1", borderRadius: 9, padding: "5px 12px", fontSize: 11, fontWeight: 800, cursor: "pointer", lineHeight: 1.1 }}>Reset Layout</button>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 2.8fr 1fr", gap: 10, alignItems: "center" }}>
            <label style={{ fontSize: 10, fontWeight: 800, color: "#475569", padding: 0, lineHeight: 1.15 }}>
              Case Numbers
              <input type="range" min="10" max="35" value={casePanelWidth} onChange={(e) => setCasePanelWidth(Number(e.target.value))} style={{ width: "100%", marginTop: 1, height: 16, display: "block" }} />
            </label>
            <label style={{ fontSize: 10, fontWeight: 800, color: "#475569", padding: 0, lineHeight: 1.15 }}>
              Report Preview
              <input type="range" min="40" max="75" value={previewPanelWidth} onChange={(e) => setPreviewPanelWidth(Number(e.target.value))} style={{ width: "100%", marginTop: 1, height: 16, display: "block" }} />
            </label>
            <label style={{ fontSize: 10, fontWeight: 800, color: "#475569", padding: 0, lineHeight: 1.15 }}>
              Case Documents
              <input type="range" min="10" max="35" value={docsPanelWidth} onChange={(e) => setDocsPanelWidth(Number(e.target.value))} style={{ width: "100%", marginTop: 1, height: 16, display: "block" }} />
            </label>
          </div>
        </div>
        ...
        ════════════════════════════════════════════════════════════════════ */}
        <div style={{ display: "flex", gap: 12, alignItems: "flex-start", minHeight: "calc(100vh - 205px)", overflowX: "auto", paddingBottom: 8 }}>
 
          {/* ── COL 1: Case list ──────────────────────────────────────────── */}
          <div style={{ width: `${casePanelWidth}%`, minWidth: 190, flexShrink: 0, display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              {displayedReports.length === 0 ? (
                ...
                <div className="qc-case-list" style={{ maxHeight: "calc(100vh - 320px)", overflowY: "auto" }}>
                  {displayedReports.map((item) => {
                    const isSent = sentCases[item.caseId] || item.sentToCRM;
                    ...
                    return (
                      ...
                      >
                        ...
                        {isSent && <div className="qc-case-badge sent">✓ Sent to CRM</div>}
                        <span
          ...
          <div style={{ width: `${previewPanelWidth}%`, minWidth: 480, flexShrink: 0, display: "flex", flexDirection: "column", gap: 10 }}>
            <div
              className="qc-card"
              style={{
                padding: 0,
          ...
          <>
            ...
            {showSendButton && (
              <div className="qc-card" style={{ border: "2px solid #6ee7b7", background: "#f0fdf4" }}>
                <h2 className="qc-section-title" style={{ color: "#064e3b" }}>🚀 Send to CRM</h2>
                {alreadySent ? (
                  <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "16px 20px", background: "#dcfce7", borderRadius: 14, border: "2px solid #86efac", color: "#166534", fontWeight: 700, fontSize: 15 }}>
                    ...
                    <div>
                      <div>Report already sent to CRM</div>
                  ...
                  <div>
                    <p style={{ color: "#065f46", fontSize: 14, marginBottom: 16, lineHeight: 1.6 }}>
                      The report has been saved and is ready to be sent to CRM.
                      Once sent, this case will move to CRM Check using Case ID {" "}
                      <strong style={{ fontFamily: "monospace" }}>{report.caseId}</strong>.
                    </p>
                    <button className="qc-btn-send" onClick={handleSendToCRM} disabled={isSending} style={{ fontSize: 15, padding: "14px 28px" }}>
                      {isSending ? (
                        <><svg style={{ width: 18, height: 18, animation: "qcSpin 0.8s linear infinite" }} viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="white" strokeWidth="4" opacity="0.3" /><path fill="white" d="M4 12a8 8 0 018-8v8z" /></svg> Sending to CRM...</>
                      ) : <>🚀 Send to CRM</>}
                    </button>
                  </div>
                )}
